function [U, S, V] = approximateSVD(X,C,ReducedDim,X_org , mode)
...
if mFea/nSmp > 1.0713
    ...
    if nargout >= 3
        ...
        V = X_org'*(U.*repmat(eigvalue_MinusHalf',size(U,1),1));
    end
else
    ddata = X'*X;
    ddata = max(ddata,ddata');
    ...
    if(mode == 2 || mode == 3 || mode == 5)
        V = C*(V.*repmat(eigvalue_MinusHalf',size(V,1),1));
    elseif(mode == 4)       
        V = C*V;
    end
    
    U = X_org*(V.*repmat(eigvalue_MinusHalf',size(V,1),1));
end
